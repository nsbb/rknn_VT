...
## FAR 평가 (test set 1897파일, 총 77분)

> ⚠️ 주의: 이 FAR 평가는 웨이크워드 포함 전체 test set을 대상으로 함.
> 실제 FAR는 비웨이크워드 전용 오디오(measure_FA/)로 재측정 필요.

| 설정 | FA 수 | Duration | FAR/hour |
|------|-------|----------|----------|
| Raw | 750 | 77 min | 584/hr |
| Refractory | 278 | 77 min | 217/hr |
| Refrac + EMA | 272 | 77 min | 212/hr |
| Refrac + EMA + N-of-M | 239 | 77 min | 186/hr |

> 파라미터: threshold=0.5, EMA α=0.3, N-of-M (3/5), Refractory=2.0s
...
- **최적 threshold**: 0.55 (F1=95.76%, FP=2, FAR~1.9/hr)
- ONNX CPU 추론(2.5ms) vs NPU(6.2ms): NPU가 느리지만 CPU 부하 오프로드 이점
- 다음 작업: measure_FA/ 기준 정밀 FAR 측정, 마이크 스트리밍 연결
